...
var buildClientBundle = require('./src/renderer/lbclient/build');
...
gulp.task('compile:scripts:watch', function (done) {
  gulp.src(['src/**/*.{js,jsx}', '!src/renderer/lbclient/**/*.js'])
    // .pipe($.watch(['src/**/*.{js,jsx}', '!src/renderer/lbclient/**/*.js'], {verbose: true}))
    .pipe($.plumber())
    .pipe($.sourcemaps.init())
    .pipe($.babel({stage: 0}))
    .pipe($.sourcemaps.write('.'))
...
// Incremental compile ES6, JSX files with sourcemaps
gulp.task('compile:lbclient', function (done) {
    buildClientBundle(process.env.NODE_ENV || 'development', function (err){
      console.log(err);
      gulp.src('src/renderer/lbclient/**/*.js')
      .pipe($.plumber())
      .pipe(gulp.dest(serveDir + '/renderer/lbclient'));
      done();
    });
});
...
gulp.task('compile:scripts', function () {
  return gulp.src(['src/**/*.{js,jsx}', '!src/renderer/lbclient/**/*.js'])
    .pipe($.babel({stage: 0}))
    .pipe($.uglify())
...
gulp.task('serve', ['compile:lbclient', 'inject:css', 'compile:scripts:watch', 'compile:styles', 'misc'], function () {
...
gulp.task('build', ['compile:lbclient', 'html', 'compile:scripts', 'packageJson', 'copy:fonts', 'misc']);
